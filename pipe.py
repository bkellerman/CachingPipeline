...
class CachingPipeline(object):
    ...
    def __init__(self, pipeline, cache_dir, verbose=0, use_cache=True):
        ...
        self.final_est_name = self.steps[-1][0]
        self.tf_params = self.get_tf_params()

    def get_tf_params(self):
        #Filter out steps, since KerasClassifier doesn't have __repr__
        tf_params = {k:v for k,v in self.pipeline.get_params().items() if k != 'steps'}
        tf_params = {k:v for k,v in tf_params.items() if k != self.final_est_name}
        tf_params = {k:v for k,v in tf_params.items() if not k.startswith(self.final_est_name + '__')}
        return str(sorted(tf_params.items()))
    ...
    def array_hash(self, X):
        ...
        hash_params = self.tf_params + data_hash
        h = hashlib.sha256()
    ...
    def load_transformers(self, data_hash):
        for i, tf_tuple in enumerate(self.pipeline.steps[:-1]):
            tf_name, tf = tf_tuple
            hash_params = str(tf.get_params()) + data_hash
            h = hashlib.sha256()
            h.update(hash_params.encode('utf-8'))
            hash_str = h.hexdigest()

            try:
                print("Loading %s from %s." % (tf_name, hash_str))
                tf_obj = joblib.load(os.path.join(self.cache_dir, tf_name + '.' + hash_str + '.pkl'))
            except Exception as e:
                print(e)
                raise IOError("Couldn't load transfomer %s from cache." % (tf_name))

            self.pipeline.steps[i] = (tf_name, tf_obj)

    def save_transformers(self, data_hash):
        for tf_name, tf in self.pipeline.steps[:-1]:
            hash_params = str(tf.get_params()) + data_hash
            h = hashlib.sha256()
            h.update(hash_params.encode('utf-8'))
            hash_str = h.hexdigest()
            try:
                print("Saving %s to %s." % (tf_name, hash_str))
                joblib.dump(tf, os.path.join(self.cache_dir, tf_name + '.' + hash_str + '.pkl'))
            except Exception as e:
                print(e)
                raise IOError("Couldn't save transfomer %s to cache." % (tf_name))
    ...
    def tf_transform(self, X):
        ...
        if X_tf is not None:
            self.load_transformers(self.array_hash(X))
            return X_tf
        ...
        self.save_transformers(self.array_hash(X))
...
class CachingPipelineY(object):
    ...
    def __init__(self, pipeline, cache_dir, verbose=0, use_cache=True):
        ...
        self.named_steps = pipeline.named_steps
        self.final_est = self.steps[-1][-1]
        self.pipe_params = str(sorted(self.pipeline.get_params().items()))
    ...
    def predict_proba(self, X):
        ...
        # This is already done in tf_tranform()
        """
        X_tf = self.get_cache(X)
